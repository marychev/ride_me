...
class GoBackgroundTest(BaseGameScreenGUITest):
    ...
    def test_go_mountains_start(self):
        self.set_app()
        old_uvpos = self.background.texture.uvpos
        self.background.is_repeat_texture = True
        self.road.go_start()
        self.background.on_go_mountains(.1)
        new_uvpos = self.background.texture.uvpos
    ...
    def test_go_mountains_stop(self):
        self.set_app()
        old_pos = self.background.texture.uvpos
        self.bike.speed = 0
        self.road.on_go(.1)
        self.road.go_stop()

        new_pos = self.background.texture.uvpos
